...
export default async function handler(req, res) {
  ...
  const memberId = texto((req.query || {}).userId || body.userId);
  ...
  try {
    ...
    if (req.method === "GET") return res.status(200).json({ membros: await portfolioMembersForUser(portfolioId, user.id) });
    ...
    if (req.method === "DELETE") {
      if (!memberId) return erro(res, 400, "membro_invalido");
